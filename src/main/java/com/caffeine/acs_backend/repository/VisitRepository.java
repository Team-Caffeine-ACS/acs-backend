...
public interface VisitRepository extends JpaRepository<Visit, UUID> {
  ...
  @Query(
      value =
          """
    SELECT v.id, v.access_level, v.access_point_id, v.arrival_time,
        ...
        v.notes, v.status, v.visitor_person_in_role_id, v.visitor_email,
        v.visitor_full_name, v.created_at
    FROM visit v
    ...
    AND (CAST(:search AS varchar) IS NULL OR LOWER(v.visitor_email) LIKE LOWER(CONCAT('%', :search, '%')))
    """,
      countQuery =
          """
