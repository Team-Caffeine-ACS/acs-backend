...
class ScheduledVisitorControllerTest {
  ...
  void shouldGetById() throws Exception {
    ...
    ScheduledVisitorResponse response =
        new ScheduledVisitorResponse(
            id, LocalDateTime.now(), "John Doe", null, "Host", VisitStatus.PRE_REGISTERED);

    when(service.getById(id)).thenReturn(response);

    mockMvc.perform(get("/scheduled-visitors/" + id)).andExpect(status().isOk());
  }
  ...
  void shouldReturnStats() throws Exception {
    ...
    mockMvc.perform(get("/scheduled-visitors/stats")).andExpect(status().isOk());
  }
  ...
  void shouldExport() throws Exception {
    ...
    mockMvc.perform(get("/scheduled-visitors/export")).andExpect(status().isOk());
  }
}
